...
if (isset($text)) {
    ...
    if ($text == '') {
        ...
        if ($num_rows = mysqli_num_rows($checkUser) > 0) {
            ...
            $response = "CON Welcome - ODM Digital Portal. \n";
            ...
            $response = "CON The ODM Digital Portal \n";
            $response .= "1. Register \n";
            $response .= "2. Quit \n";
        }
    }

//SUBSCRIBING TO ODM ALERTS
...
//Save subscription details into the DataBase
    ...
    if (isset($level[0]) && $level[0] == 2 && isset($level[1]) && $level[1] == 4 && $registeredUser) { //
        ...
        $response = "END Thank you for using the ODM Portal, Tibim!";
    ...
    if (isset($level[0]) && $level[0] == 4 && isset($level[1]) && $level[1] == 2 && $registeredUser) {
        ...
        $response = "END Thank you for accessing the ODM Portal. We look forward to our continued cooperation! Tilalala! \n";
...
//END
    ...
    if (isset($level[0]) && $level[0] == 5 && isset($level[1]) && $registeredUser) { //
        ...
        if ($balance < $level[1]) {
            $response = "END You do not have sufficient funds in your wallet to donate Ksh.$level[1] to the ODM Party. Wallet Balance: Ksh.$balance !";
        } else {
            ...
            if ($donate) {
                ...
                $response = "END You have successfully donated Ksh.$level[1] to the ODM Party, Your wallet balance is Ksh.$new_wallet_balance Thank you!";
    ...
    if (isset($level[0]) && $level[0] == 6 && !isset($level[1]) && $registeredUser) {
        ...
        if ($fname == '' || $other_names == '' || $voters_no == '' || $member_no == '' || $doc_no == '' || $doc_type == '' || $county == '' || $constituency == '' || $ward == '' || $polling_station == '') {
            ...
            $response = "END Your profile is INCOMPLETE, Visit ODM Web Portal to provide: $fname $other_names $voters_no $member_no $doc_no $doc_type $county $constituency $ward $polling_station  ";
...
//Quiting the main menu
    if (isset($level[0]) && $level[0] == 8 && !isset($level[1]) && $registeredUser) { //
        ...
        $response = "END Thank you for accessing the ODM Portal. We look forward to our continued cooperation! \n";
...
//End
    ...
    if (isset($level[0]) && $level[0] != '' && $level[0] == 1 && !isset($level[1]) && !$registeredUser) {
        ...
        $response = "CON Enter PIN for ODM portal. \n";
     ...
     values('$m_user_id', '$level[1]', '$level[2]', '$level[3]', '1', '1', '$pwd', '$pwd', '$phoneNumber', '$level[4]', '1', '0', '$date', '$expiry_date', '1', '1' )") or die(mysqli_error($conn));
        ...
        $response = "END Thank you $level[1] $level[2] for registering with the ODM Party. Please proceed to members.odm.co.ke to update your profile. Welcome!";
    }
    ...
    if ($registeredUser && $level[0] == 1 && isset($level[1]) && $level[1] == 14 && !isset($level[2])) {
        //*1*14#
        $response = "CON Enter current ODM Portal PIN. \n";
    }
    if ($registeredUser && $level[0] == 1 && isset($level[1]) && $level[1] == 14 && isset($level[2]) && !isset($level[3])) {
        ...
        if ($user_rows = mysqli_num_rows($checkPin) > 0) {
            $response = "CON Enter new ODM Portal PIN. \n";
        } else {
            $response = "END Incorrect PIN ";
        }
    }
    if ($registeredUser && $level[0] == 1 && isset($level[1]) && $level[1] == 14 && isset($level[2]) && isset($level[3]) && !isset($level[4])) {
        ...
        if ($user_rows = mysqli_num_rows($checkPin) > 0) {
            ...
            $response = "CON Confirm new ODM Portal PIN. \n";
        } else {
